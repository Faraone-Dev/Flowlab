...
/// Field order chosen so natural alignment gives exactly 40 bytes; no packing needed.
struct Event {
    uint64_t ts;
    uint64_t price;
    uint64_t qty;
...
/// Fixed-depth order book for hot path operations.
/// Uses sorted arrays (not trees) for cache efficiency.
...
public:
    ...
    uint64_t best_bid() const noexcept {
        return bid_count_ > 0 ? bids_[0].price : 0;
    }

    /// Best ask (lowest price)
    uint64_t best_ask() const noexcept {
